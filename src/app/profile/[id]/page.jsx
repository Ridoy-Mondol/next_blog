...
function Page({ params }) {
  ...
  const { posts, loading, author, getUser, getSinglePost, singlePost } =
    usePosts();
      ...
      if (name.length > 0 || img) {
        ...
        try {
          ...
          const res2 = await fetch(
            `/api/users/blog/${posts.map((post) => post._id)}`,
            {
              method: "PATCH",
              headers: headers,
              body: formData,
      ...
      <div className="profile-div">
        ...
        <div className="profile-card">
          <div className="profile-image" onClick={() => setShowImg(true)}>
            <Image
              src={user.profileImage != null ? user.profileImage : profileImage}
              alt="Profile"
              width="1000"
              height="1000"
            />
          ...
          <div className="profile-info">
            <h2 className="text-capitalize">
              ...
              <FontAwesomeIcon
                icon={faPen}
                className={`ml-2 ${user._id === author ? "" : "d-none"}`}
                onClick={() => setShow(true)}
              />
            </h2>
            <p>
              Total Posts:{" "}
              {posts.filter((post) => post.user?.author === user._id).length}
            </p>
          </div>
        </div>

        <div
          className={`position-absolute update-name ${
            showImg && user._id === author ? "" : "d-none"
          }`}
        >
          ...
          <input
            type="file"
            accept="image/*"
            className="update-name-input update-img"
            onChange={(e) => setImg(e.target.files[0])}
          />
          <div className="update-name-btn">
            <button className="input-btn save-btn" onClick={updateInfo}>
              Save
            </button>
            <button
        ...
        <div
          className={`position-absolute update-name ${show ? "" : "d-none"}`}
        >
        ...
        <div className="profile-parent">
          ...
          <div className="flex justify-between flex-wrap gap-8">
            {posts.map((val) => {
              if (
                (val.user.author != null || singlePost.user?.author != null) &&
                val.user?.author === singlePost.user?.author
              ) {
                return (
                  <Link
                    href={`/articles/${val._id}`}
                    key={val._id}
                    className=" position-relative user-post"
                  >
                    {" "}
                    <div>
