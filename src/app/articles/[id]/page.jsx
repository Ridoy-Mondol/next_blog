...
function Page({ params }) {
  ...
  const { id } = params;
  const { posts, loading, author, deletePost, getSinglePost, singlePost } =
    usePosts();
      ...
      <div>
        ...
        <div>
          ...
          <div className="flex text-s single-blog">
            ...
            <div className="single-para-parent">
              <div
                className="single-para"
                dangerouslySetInnerHTML={{ __html: singlePost?.blog }}
              />
              <div className="flex justify-between align-items-center pt-12 pb-16 text-xs">
                <Link
                  href={`/profile/${singlePost?._id}`}
                  className="flex align-items-center space-x-2"
                >
                  <Image
                    src={
                      singlePost?.user?.profileImage != null
                ...
                <div
                  className={`position-absolute list-none editpart2 d-flex flex-column justify-content-center space-y-4 ${
                    showbox
                      ? author === singlePost?.user?.author
                        ? "show-box"
                        : "show-sm"
                      : ""
                  }`}
                >
                  ...
                  <Link
                    href={`/post/edit/${singlePost?._id}`}
                    className={
                      singlePost?.user?.author === author ? "" : "d-none"
                    }
                  >
                  ...
                  <li
                    onClick={() =>
                      deleteItem(singlePost?._id, singlePost?.user.author)
                    }
                    className={
                      author === singlePost?.user?.author ? "" : "d-none"
                    }
                  >
                ...
                <div
                  className={`editonclick d-flex align-items-center justify-center text-xs`}
                  onClick={() => Showbox()}
                >
                  <FontAwesomeIcon
                    icon={faEllipsisV}
        ...
        <div className="single-parent">
          <h3 className="my-3 font-semibold popular-title single-title">
            {posts.filter(
              (post) =>
                post.category === singlePost?.category &&
                post._id !== singlePost?._id
            ).length > 0 && "Related Posts"}
          </h3>
          <div className="flex flex flex-wrap popular-blogs single-blogs-div">
            {posts.map((val) => {
              if (
                val.category === singlePost?.category &&
                val._id !== singlePost?._id
              ) {
                return (
                  <Link
                    href={`/articles/${val._id}`}
                    key={val._id}
                    className=" position-relative single-div"
                  >
                    {" "}
                    <div>
                    ...
                    <div className="position-absolute top-0 pt-24 pl-4 w-100 h-100 single-abs">
                      ...
                      <div className="flex align-items-center justify-start space-x-2 text-xs popular-date text-white">
                        ...
                        <span>{val.date.split("T").at(0)}</span>
                      </div>
                    </div>
                  </Link>
                );
